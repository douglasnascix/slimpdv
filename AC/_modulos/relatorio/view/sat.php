...
<form action="<?php echo $url;?>relatorio/sat/" method="POST">
<div class="col-lg-12">
...
<div class="col-lg-12">
	<div class="panel panel-default">
		<div class="panel-heading"><b>Relatório</b></div>
			<div class="panel-body">
				<div class="row legenda-tabela">
					<div class="col-sm-12">
						<div class="col-sm-3 bg-success">Emitido</div>
						<div class="col-sm-3 bg-danger">Cancelado</div>
						<div class="col-sm-3 bg-warning">Falha</div>
						<div class="col-sm-3 ">Aguardando</div>
					</div>
				</div>
						...
						<tbody>
							...
							switch($cupom['cupom_status']){
								case 'Falha':
									...
									$status_btn = '<a class="btn btn-primary" data-toggle="tooltip" title="'.$cupom['cupom_status'].' | '.$cupom['EEEEE'].' | '.$cupom['CCCC'].' | '.$cupom['mensagem'].'"><i class="fa fa-info-circle"></i></a> ';
									$status_btn .= '<a href="'.$url.'_modulos/caixa/src/sat.php?acao=emitir&pedido_id='.$cupom['pedido_id'].'" class="btn btn-success"><i class="fa fa-refresh"></i></a>';
									break;
								case 'Emitido':
							...
							<tr <?php echo $status_cor; ?> >
								<td width="12%"><?php echo $status_btn?>
									<?php if($cupom["cupom_status"] == "Emitido"){
										if(calcula_data($cupom["timeStamp"]) <= 28){
											echo ' <a href="'.$url.'_modulos/caixa/src/sat.php?acao=cancelar&chaveConsulta='.$cupom['chaveConsulta'].'" class="btn btn-danger"><i class="fa fa-ban"></i></a>';
										};
									}?>
								</td>
								<td><?php echo $cupom['pedido_id']; ?></td>
								<td><?php echo $cupom['CPFCNPJValue']; ?></td>
